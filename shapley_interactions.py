...
class Shapley_Interactions:
    ...
    def kernel_q(self,t,sampling_kernel):
        if sampling_kernel == "ksh":
            return np.math.factorial(self.n-t-self.s)*np.math.factorial(t-self.s)/np.math.factorial(self.n-self.s+1)
    ...
    def kernel_m(self,t):
        if self.type=="SII":
            return np.math.factorial(self.n - t - self.s) * np.math.factorial(t) / np.math.factorial(self.n - self.s + 1)
        if self.type=="STI":
            return self.s * np.math.factorial(self.n - t - 1) * np.math.factorial(t) / np.math.factorial(self.n)
        if self.type=="SFI":
            return np.math.factorial(2*self.s-1)/np.math.factorial(self.s-1)**2 * np.math.factorial(self.n - t - 1) * np.math.factorial(t+self.s-1) / np.math.factorial(self.n+self.s-1)
...
total_subsets = 2**n
budget_list = [int(total_subsets*0.01),int(total_subsets*0.1),int(total_subsets*0.25),int(total_subsets*0.5),int(total_subsets*0.75),int(total_subsets*0.90)]
...
for shapx in shapx_list:
    ...
    for budget in budget_list:
        if shapx.type in ("STI","SII"):
            ...
            approximation_errors[id + "_perm"] = np.sum((shapx_perm[id] - shapx_exact[shapx.type][shapley_interaction_order]) ** 2)
        ...
        for sampling_kernel in sampling_kernel_list:
            for pairwise in pairwise_list:
                ...
                shapx_sampling[id] = shapx.compute_interactions_from_budget(game_fun, budget, pairing=pairwise,sampling_kernel=sampling_kernel)
                approximation_errors[id] = np.sum((shapx_sampling[id][shapley_interaction_order] - shapx_exact[shapx.type][shapley_interaction_order]) ** 2)
                approximation_errors[id+"_full"] = np.sum((shapx.last_const_complete[shapley_interaction_order] - shapx_exact[shapx.type][shapley_interaction_order]) ** 2)
