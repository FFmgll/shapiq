...
if __name__ == "__main__":
    ...
    N_NON_IMPORTANT_FEATURE_RATIOS: list = [0.05,0.1,0.25,0.5,0.75,0.9,0.95]

    time_id = str(int(time.time()))

    for n_non_important_feature_ratio in N_NON_IMPORTANT_FEATURE_RATIOS:
        print(f"SLN with {n_non_important_feature_ratio*100}% unimportant features")
        game = ParameterizedSparseLinearModel(n=N_FEATURES, weighting_scheme="uniform",
                                              n_interactions=100, n_non_important_features=int(N_FEATURES*n_non_important_feature_ratio))
                    ...
                    for interaction_type, approximator in approximators.items():
                        ...
                        for budget in budgets:
                            ...
                            approximated_interactions = copy.deepcopy(
                                baseline_approximator.approximate_with_budget(game_fun, budget,SHAPLEY_INTERACTION_SUBSETS))
                            ...
                            for sampling_kernel in SAMPLING_KERNELS:
                                ...
                                for pairwise in PAIRWISE_LIST:
                                    ...
                                    approximation_errors[
                                        '_'.join((run_id3, 'const and sampling',str(approximator.last_sampling_params["sampling"]),str(approximator.last_sampling_params["average_std"]),str(approximator.last_sampling_params["std_threshold"])))] = get_approximation_error(
        ...
        # Store All ------------------------------------------------------------------------------------
        save_name = "_".join((time_id,game_name, str(n_non_important_feature_ratio), str(N_FEATURES), str(SHAPLEY_INTERACTION_ORDER))) + ".csv"
        approx_errors_df = pd.DataFrame(approx_errors_list)
        approx_errors_df.to_csv(os.path.join("results/sln", save_name), index=False)

        # Plot run -------------------------------------------------------------------------------------
        plot_title = " ".join((time_id,game_name,str(n_non_important_feature_ratio), str(N_FEATURES), str(SHAPLEY_INTERACTION_ORDER)))
        #data = pd.read_csv(os.path.join("results/sln","1674564475_sparse_linear_model_0.1_30_2.csv"))
