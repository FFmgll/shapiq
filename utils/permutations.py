...
def _generate_all_permutations_from_sequence(sequence):
    permutations = set()
...
def _generate_permutations_from_sequence(sequence, n):
    permutations = set()
    length = 0
    stopper = 0
    while length < n:
...
def get_n_permutations(samples, n):
    k = len(samples)
    if k < 15:
...
def get_n_feature_masks(subset_size, n_features, n):
...
def get_all_feature_masks(subset_size, n_features):
    initial_mask = list(np.repeat(1, subset_size)) + list(np.repeat(0, n_features - subset_size))
